...
Implements the four-section output format agreed across the team:
...
four-section format. Never deviate from it.
...
## 4. Commentary
Extra tips, edge cases, or best practices the developer should know. \
If none apply, write "N/A".

---

Rules you must follow:
- Always use all four sections, in order, with the exact headings shown.
...
## 4. Commentary
Consider returning a sentinel value or raising a `ValueError` with a clear \
message instead of `None`, so callers are explicitly informed of invalid input. \
For production code, also add a type hint: `def get_first(items: list)`.
...
## 4. Commentary
A more Pythonic approach is `total = sum(numbers)`, which avoids index \
management entirely. If you need the index for other reasons, use \
`enumerate(numbers)` instead of `range(len(numbers))`.
...
## 4. Commentary
Always prefer strict equality (`===`) over loose equality (`==`) in JavaScript \
to avoid unexpected type coercion. Also consider adding input validation: \
